...
def create_simulation(
    ...
    images_seq_shape = (sim_observations, *initial_conditions[0].shape)
    time_shape = (sim_observations,)
    params_shape = params_provider(1).shape[0:]
    ...
    with export.get() as e:
        ...
        for _ in range(rounds):
            for u in ics:
                us = [u]
                ts = [th.zeros((time_shape))]
                p = params_provider(len(u))

                for _ in range(sim_observations):
                    steps = th.randint(min_sim_steps, max_sim_steps, size=(batch_size,))
